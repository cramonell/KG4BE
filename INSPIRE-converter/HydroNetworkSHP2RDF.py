...
INSPIRE_HN= Namespace("https://example.org/INSPIRE/HydroNetwork#")
...
def convert(infile, id_field=None, data_ns=None, schema_ns=None, include_wgs84=True, include_relations=False):
    ...
    g.bind("hydroNetwork", data_ns)
    g.bind("hydroProps", schema_ns)
    g.bind('inspireF', INSPIRE_FC)
    ...
    for feature_index in range(len(shape['features'])):
        ...
        g.add((feature_id, RDF['type'], INSPIRE_FC['HydroLink']))

       

        for field_key in fields.keys():
            if field_key == 'id_curso':
                if fields[field_key] not in linksets:
                    linksets.append(fields[field_key])
                    g.add((data_ns[str(fields[field_key])], RDF['type'] , INSPIRE_FC['LinkSet'] ))
                else: pass
...
schema_ns = 'http://www.example.org/Government/Networks/Hydro/HydroNetworkPorperties/ontology#'
data_ns = 'http://www.example.org/Government//Networks/Hydro/HydroNetwork/data/'
conversion_object = convert(infile="D:/SemTech/GeoData/Carreteras/ABOVE_A2_HIDROGRAPHY.shp", schema_ns=schema_ns, data_ns=data_ns, include_relations=False)
conversion_object.write("A2_Above_Hydrography.ttl")
