...
INSPIRE_RN= Namespace("https://example.org/INSPIRE/RoadNetwork#")
...
def convert(infile, id_field=None, data_ns=None, schema_ns=None, include_wgs84=True, include_relations=False):
    ...
    for feature_index in range(len(shape['features'])):
        ...
        g.add((feature_id, RDF['type'], INSPIRE_FC['RoadLink']))

       

        for field_key in fields.keys():
            if field_key == 'id_vial':
                if fields[field_key] not in linksets:
                    linksets.append(fields[field_key])
                    g.add((data_ns[str(fields[field_key])], RDF['type'] , INSPIRE_FC['Road'] ))
                else: pass
        ...
        if geometry:
            ...
            g.add((geom_id, GEO_NS['asGeoJson'], Literal(json.dumps(geometry),datatype=GEO_NS['geoJSONLiteral'])))
...
schema_ns = 'http://www.example.org/Government/Networks/TransportNetworks/RoadTransportNetworkPorperties/ontology#'
data_ns = 'http://www.example.org/Government//Networks/TransportNetworks/RoadTransportNetwork/data/'
conversion_object = convert(infile="D:/SemTech/GeoData/FFCC/ABOVE_ROAD.shp", id_field="id_tramo", schema_ns=schema_ns, data_ns=data_ns, include_relations=False)
conversion_object.write("LAV_PLASENCIA_BADAJOZ_Above_Road.ttl")
